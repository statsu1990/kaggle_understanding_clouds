...
def dice_coef(y_true, y_pred, smooth=1):
    ...
    y_pred_f = K.flatten(K.relu(y_pred))
...
def post_process_closing(probability, kernel_size=20, n_open=0, n_close=1):    
    kernel = np.ones((kernel_size, kernel_size),np.uint8)
    posted = probability.copy()
    for i in range(n_open):
        posted = cv2.morphologyEx(posted, cv2.MORPH_OPEN, kernel)
    for i in range(n_close):
        posted = cv2.morphologyEx(posted, cv2.MORPH_CLOSE, kernel)
    return posted
    ...
    for i in range(0, df.shape[0], 300):
        ...
        for class_index in range(N_CLASSES):
            ...
            for index in range(len(batch_idx)):
                ...
                sample_pred_post = post_process_closing(sample_pred_post)
...
def save_train_val_test_images(save_dir='../proc_input', seed=None, test_size=0.2, HEIGHT=384, WIDTH=480):
    # Load data
    ...
    X_train, X_val = train_test_split(train_df, test_size=test_size, random_state=seed)
    X_train['set'] = 'train'
    ...
    train_images_dest_path = save_dir + '/train_images/'
    validation_images_dest_path = save_dir + '/validation_images/'
    test_images_dest_path = save_dir + '/test_images/'
...
def gamma_correction(imgs, gamma=0.8):
    g_imgs = imgs * (imgs / 255) ** gamma
    g_imgs = g_imgs.astype(imgs.dtype)
    return g_imgs
